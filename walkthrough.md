...
- **Uygulama Sürüm Kodu**: `35`
- **Uygulama Sürüm Adı**: `1.3.5`
...
- **Silinen Hesapların Tam Temizlenmesi (`purgeUserFirestoreData`)**: Silinen hesapların yeniden giriş yapıldığında eski resim/mesaj kalıntılarının yüklenmesi engellendi. `ProfileService` ve `AuthService` içerisine `purgeUserFirestoreData(uid)` eklenerek kullanıcı silindiğinde Firestore `users/{uid}`, alt koleksiyonlar (`swipes`, `stats`, `credit_transactions`), sohbet odaları (`chats`), mesajlar, beğeniler (`likes`) ve eşleşmeler (`matches`) tamamen kazınır.
- **Admin Paneli Medya Tarayıcı Düzeltmesi (`mediaService.ts`)**: Admin panelindeki Medya sayfasında boş/yüklenmeme sorunu giderildi. Firestore kullanıcı profil fotoğrafları (`photoUrls`), video profili (`videoUrl`) ve ses kayıtları (`profileVoiceUrl`) hibrit tarama ile Medya paneline bağlandı.
